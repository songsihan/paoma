var SelectScroll = (function (_super) {
    __extends(SelectScroll, _super);
    ...
    var d = __define,c=SelectScroll;p=c.prototype;
    p.OnScrollSelectBegin = function (event) {
        this.imgBgSelected.visible = false;
        this.beginScrollY = event.localY;
    };
    p.OnScrollSelectMove = function (event) {
    ...
    p.OnScrollSelectEnd = function (event) {
    ...
    p.GetPreIdx = function (roleIdx) {
        if (roleIdx == 0) {
            return 5;
        }
        return roleIdx - 1;
    };
    p.GetNextIdx = function (roleIdx) {
        if (roleIdx == 5) {
            return 0;
        }
        return roleIdx + 1;
...
egret.registerClass(SelectScroll,"SelectScroll");
