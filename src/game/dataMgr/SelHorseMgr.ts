 ...
 */
module game {
    export class SelHorseMgr {
        private static instance: SelHorseMgr;
        private pushMoenyList: Array<any> ; // 压钱的队列
        private selPalyIndex: number; // 选中的玩法
        private list: Array<any> = new Array(2); // 马跑的轨迹数据
        static horseNum: number = 5; // 马的数量
        static viewHorseIndex: number = 3; // 场景跟随的索引
        private selStart: game.SELECT_INDEX; // 选择几星玩法
        ...
        public static GetInstance(): SelHorseMgr
        {
            if (!this.instance)
            {
        ...
        public setPushMoeny(index:number, money:number)
        {
           
            this.pushMoenyList[index] = this.pushMoenyList[index] + money;
        }
        
        public getPushMoney(index:number):number
        {
            return this.pushMoenyList[index]
        }
        
        public setSelStar(index:game.SELECT_INDEX)
        {
            this.selStart = index;
        }
        
        // 设置马的配置数据
        private setHorseMapInfo()
        {
            var map1 = 
            {
                r: 460,
